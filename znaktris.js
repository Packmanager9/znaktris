...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    // for(let t = 0;t<50;t++){
    //     if(Math.random()<.5){
    //         let ball = new Circle(Math.random()*700,Math.random()*700, 5, "red")
    //         ball.type = 1
    //         ball.friction = .98
    //         orbs.push(ball)

    //     }else{
    //         let ball = new Circle(Math.random()*700,Math.random()*700, 5, "cyan")
    //         ball.type = 0
    //         ball.friction = .98
    //         orbs.push(ball)
    ...
    class Oblock {
        constructor(point){
            ...
            this.clear = true
            for(let t = 0;t<4;t++){
                    let point = new Circle(this.center.x+(Math.cos(angle)*8*1.41), (this.center.y+(Math.sin(angle)*8*1.41)), 3, "Yellow")
        ...
        dreverse(){
            ...
            // board.play = 0
                ...
                for(let t = 0; t < this.blocks.length;t++){
                    this.tiles.push(board.compare(this.blocks[t]))
                    for(let k = 0;k<this.tiles.length;k++){
                        this.tiles[k].occupied = 1
                        this.tiles[k].king = this
                        this.tiles[k].queen = this.blocks[t]
                    }
                    this.blocks[t].x-=16
                }
        }
        ...
        draw(){
            if(this.locked == 0){
                ...
                let wet = 0
                for(let t = 0; t < this.blocks.length;t++){
                    if(board.compare(this.blocks[t]).occupied == 1 && board.compare(this.blocks[t]).king != this){
                        wet = 1
                    }
                }
                if(wet == 0){
                    ...
                    if(keysPressed['a']){
                        if(this.center.x > 376){
                            for(let t = 0;t<this.tiles.length;t++){
                                this.tiles[t].occupied = 0
                            }
                        this.center.x-=16
                        this.tiles = []
                        for(let t = 0; t < this.blocks.length;t++){
                        ...
                        for(let n = 0; n < this.blocks.length;n++){
                            if(board.compare(this.blocks[n]).occupied == 1 && board.compare(this.blocks[n]).king != this){
                                this.areverse() 
                                break
                            }
                        }
            ...
            if(this.locked == 0){
                ...
                if(wet == 0){
                    for(let t = 0; t < this.blocks.length;t++){
                        if(board.compare(this.blocks[t]).occupied == 1){
                            this.locked = 1
                            board.play = 0
                            break
                        }
    ...
    class Grid {
        constructor(){
            ...
            for(let t = 0;t<10;t++){
                this.grid.push([])
                for(let k = 0;k<40;k++){
                    let square = new Rectangle(360+(t*16),40+ (k*16), 16, 16, `rgb(${t*5}, ${k*3.1}, ${(k*t)%50})`)
                    square.t = t
                    square.k = k
                    this.grid[t][k] = square
                }
            }
        }
        compare(point){
        ...
        draw(){
            ...
            for(let t = 0;t<10;t++){
                for(let k = 0;k<40;k++){
                    if(this.grid[t][k].occupied == 1){
                    this.grid[t][k].color = "#000044"
                    }
                    if(this.grid[t][k].occupied == 0){
                    this.grid[t][k].color = "#440000"
                    }
                    this.grid[t][k].draw()
                }
            }
            ...
            for(let k = 0;k<40;k++){
                ...
                for(let t = 0;t<10;t++){
                    if(this.grid[t][k].occupied == 1){
                        if(this.grid[t][k].king.locked == 1){
                        count++
                        }
                    }
                }
            ...
            for(let k = 0;k<rows.length;k++){
                for(let t = 0;t<10;t++){
                    ...
                    if(this.grid[t][rows[k]].occupied == 1){
                        this.grid[t][rows[k]].king.blocks.splice(this.grid[t][rows[k]].king.blocks.indexOf(this.grid[t][rows[k]].queen),1)
                        // this.grid[t][rows[k]].king.center.y -=16
                        this.grid[t][rows[k]].occupied = 0
                        this.grid[t][rows[k]].king = {}
                    }
                }
            }
            ...
            if(keysPressed['l']){
                if(this.play != 1){
                    this.play = 1
                    let  point = new Point(this.grid[5][0].x, this.grid[5][0].y+16)
                    let oblock = new Oblock(point)
                    this.tets.push(oblock)
                }
            }
        }
    }
...
class Deconstructor{
    ...
    draw(x,y){
        ...
        canvas_context.moveTo(3504, 2406)
        canvas_context.strokeStyle = "red"
        canvas_context.lineWidth = 100
        canvas_context.stroke()
        canvas_context.drawImage(eye, this.x, this.y, 20,20, x, y, 20, 20)
    }
}
...
let sumtotal = (3050*2400)
let boxes = []
for(let t = 0;t<Math.floor(sumtotal/25);t++){
    let decon = new Deconstructor((t*20)%3050,Math.floor(t)%2400)
    boxes.push(decon)
}
